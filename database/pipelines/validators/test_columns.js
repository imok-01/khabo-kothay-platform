...
async function getSupabaseColumns() {
  ...
  if (spec && spec.definitions) {
    for (const [key, def] of Object.entries(spec.definitions)) {
      if (def.properties) {
        tables[key] = Object.keys(def.properties);
      }
    }
  }
  return tables;
...
async function main() {
  ...
  for (const e of ENTITIES) {
    ...
    const missing = dbCols.filter(c => !csvHeaders.includes(c));
    const extra = csvHeaders.filter(c => !dbCols.includes(c));
    
    console.log(`\n=== ${e.table} ===`);
    console.log(`1. CSV columns: ${csvHeaders.join(', ')}`);
    console.log(`2. Supabase columns: ${dbCols.length ? dbCols.join(', ') : 'NOT FOUND'}`);
    console.log(`3. Missing from CSV: ${missing.join(', ') || 'None'}`);
    console.log(`4. Extra in CSV: ${extra.join(', ') || 'None'}`);
    
    if (dbCols.length === 0 || extra.length > 0) ready = false;
  }
